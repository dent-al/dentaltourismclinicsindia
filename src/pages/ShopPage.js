...
const ShopPage = () => {
  ...
  const enhancedProducts = products.map(product => ({
    ...
    dentalProblems: product.id === 1 ? ["Tooth Sensitivity", "Enamel Erosion"] :
                   product.id === 2 ? ["Gum Disease/Gingivitis", "Bad Breath (Halitosis)", "Bleeding Gums"] :
                   product.id === 3 ? ["Children's Dental Care", "Plaque & Tartar Buildup"] :
                   product.id === 4 ? ["Tooth Staining/Yellowing", "Plaque & Tartar Buildup", "Bad Breath (Halitosis)"] : [],
    ...
    <div className="min-h-screen bg-[#f7f7f7] w-full flex flex-col items-center py-8 px-2">
      <div className="w-full max-w-7xl flex flex-col md:flex-row gap-8">
        ...
        <aside className="bg-white rounded-2xl shadow-lg p-6 w-full md:w-72 mb-6 md:mb-0">
          <h3 className="text-xl font-bold text-[#2C73D2] mb-4">Filters</h3>
          ...
          <div className="mb-4">
            <div className="flex items-center justify-between font-semibold text-[#2C73D2] mb-2 cursor-pointer" onClick={() => toggleGroup('DentalProblems')}>
              <span>Filter by Dental Problems</span>
            ...
            {openGroups.DentalProblems && (
              <div className="flex flex-col gap-2 ml-2 max-h-48 overflow-y-auto">
                {dentalProblems.map(problem => (
                  <label key={problem} className="flex items-center gap-2 text-sm cursor-pointer hover:text-[#2C73D2]">
                    <input 
                      type="checkbox" 
            ...
            <div className="mt-4 p-3 bg-blue-50 rounded-lg">
              ...
              <div className="flex flex-wrap gap-1">
                {selectedProblems.map(problem => (
                  <span key={problem} className="text-xs bg-[#2C73D2] text-white px-2 py-1 rounded-full">
                    {problem}
                  </span>
                ))}
              </div>
        ...
        <section className="flex-1">
          ...
          <div className="flex justify-between items-center mb-6">
            <h2 className="text-xl font-bold text-[#2C73D2]">
              Dental Products ({filteredProducts.length} found)
            </h2>
          </div>
            ...
            {filteredProducts.map(product => (
              <div key={product.id} className="bg-white rounded-2xl shadow-lg border border-[#2C73D2]/10 p-4 flex flex-col items-center w-full max-w-xs mx-auto hover:shadow-xl transition-shadow">
                ...
                <div className="flex flex-wrap gap-1 mb-3 justify-center">
                  {product.dentalProblems.slice(0, 2).map(problem => (
                    <span key={problem} className="text-xs bg-blue-100 text-[#2C73D2] px-2 py-1 rounded-full">
                      {problem}
                    </span>
                  ))}
