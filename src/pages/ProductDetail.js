...
const ProductDetail = () => {
  ...
  // ✅ All hooks must be declared at the very top using named imports
  const [imgIndex, setImgIndex] = useState(0);
  const [product, setProduct] = useState(null);
  const [isLoading, setIsLoading] = useState(true);

  // Use useEffect to find the product instead of doing it directly in render
  useEffect(() => {
  ...
  // ✅ Safe images setup (won't break if product is null)
    ...
    return (
      <div className="min-h-screen bg-[#f7f7f7] flex items-center justify-center">
        <div className="text-center">
          <h2 className="text-2xl font-bold text-[#2C73D2]">Product Not Found</h2>
    ...
    <div className="min-h-screen bg-[#f7f7f7] flex flex-col items-center py-8 px-2">
      <div className="w-full max-w-4xl bg-white rounded-2xl shadow-lg p-8 flex flex-col md:flex-row gap-8">
                ...
                {images.map((img, idx) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded border-2 ${
                      imgIndex === idx ? "border-[#2C73D2]" : "border-gray-300"
              ...
              <div className="flex flex-col gap-3">
                {product.reviews.map((review, idx) => (
                  <div key={idx} className="bg-gray-100 rounded-lg p-3">
                    <div className="font-semibold text-[#2C73D2]">
                      {review.user}
                    </div>
        ...
        <div className="flex-1 flex flex-col justify-center">
          <h2 className="text-2xl font-bold text-[#2C73D2] mb-2">
            {product.name}
          </h2>
          <div className="text-lg text-[#F4A300] font-bold mb-2">
            {product.price}
          </div>
